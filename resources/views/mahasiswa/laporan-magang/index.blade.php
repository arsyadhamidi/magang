            ...
            <div class="mb-3">
                <div class="card">
                    <div class="card-header">
                        <a href="{{ route('mahasiswa-laporanmagang.create') }}" class="btn btn-primary">
                            ...
                            <tbody>
                                @foreach ($laporans as $data)
                                    <tr>
                                        ...
                                        <td class="d-flex">
                                            @if ($data->status == 'Periksa' || $data->status == 'Ditolak')
                                                <a href="{{ asset('storage/' . $data->file_magang) }}"
                                                    class="btn btn-sm btn-info mx-2" target="_blank">
                                                    <i class="fas fa-download"></i>
                                                </a>
                                            @else
                                                <a href="{{ route('mahasiswa-laporanmagang.edit', $data->id) }}"
                                                ...
                                                <form action="{{ route('mahasiswa-laporanmagang.destroy', $data->id) }}"
                                                    method="POST">
                                                    @csrf
